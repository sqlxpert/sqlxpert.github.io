...
For a specific example, consider the major update from version 1 to version 2 of the AWS command-line interface (CLI). Much of the work involved covering up different date and time representations used by different AWS services. In version 1, customers paid for the lack of consistency. Eventually, AWS paid, by [having version 2 of the CLI convert outputs to ISO 8601 date and time formats](https://docs.aws.amazon.com/cli/latest/userguide/cliv2-migration-changes.html#cliv2-migration-timestamp).
